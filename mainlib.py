...
if platform.system() == "Windows":
    print("Running on Windows - Importing extra libraries")
    import win32gui
    import win32con
import numpy as np
...
name = "2048 AI Version - Gesture Recognization"
resX = 800
resY = 600
background = (50, 50, 50); 
...
def drawBox(num,x,y):
    ...
    pygame.draw.rect(gameDisplay,color,pygame.Rect(startX+x*spacing,startY+y*spacing,sizeBox,sizeBox),border_radius=10)
...
def settingConfig():
    ...
    while not quitSet:
        ...
        if c == "1":
            ...
            if inputStr.isdigit():
                numBoxesX = math.floor((int(inputStr) - (startX * 2)) / (sizeBox + spaceBox))
            else:
                print(bcolors.WARNING + "Keeping Current Values for width")
            expandArray()
            inputStr = input("What is your monitor's Height?: ")
            if inputStr.isdigit():
                numBoxesY = math.floor((int(inputStr) - (startY * 2)) / (sizeBox + spaceBox))
...
def updateGame(currentGesture):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            initDisplay()
    global prevGest
    if prevGest != currentGesture:
        if currentGesture == "Closed_Fist":
            checkSqaures(-1,0)
        elif currentGesture == "Thumb_Up":
            checkSqaures(0,-1)
        elif currentGesture == "Thumb_Down":
            checkSqaures(0,1)
        elif currentGesture == "Victory":
            checkSqaures(1,0)
    prevGest = currentGesture
    if waterfall:
        checkSqaures(0,1)
    pygame.display.update()